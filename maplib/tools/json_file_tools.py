...
def init_tex_file_dict(tex_file_dict):
...
def get_single_tex(tex_obj, generate_file):
    ...
    if generate_file:
        ...
        if result is not None:
            ...
            tex_obj = None
    if params.PRINT_FILE_MODIFYING_MSG:
        print(params.SINGLE_TEX_MSG.format(tex_string, result_msg))
    return tex_obj


@timer_decorator()
def modify_tex_json(*string_tuples):
    """
    ...
    global_tex_dict = get_global_tex_dict()
    if params.PRINT_FILE_MODIFYING_MSG and generated_tex_objs:
    ...
    global_tex_dict = generate_tex_in_json(
        filtered_generated_tex_objs, global_tex_dict
    )
    if params.PRINT_FILE_MODIFYING_MSG and removed_tex_objs:
    ...
    global_tex_dict = remove_tex_in_json(
        filtered_removed_tex_objs, global_tex_dict
    )
    dump_tex_dict(global_tex_dict)
